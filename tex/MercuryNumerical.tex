...
This was already demonstrated in the "Schülderakademie Teilchenphysik", where this course was tested successfully on two groups consisting of in total 24 
German high school students from 10$^{\rm th}$ to 13$^{\rm th}$ grade.
...
\label{eq:newton_art}
\end{equation}
For the system at hand $a^2$ may be estimated from the parameters of mercury at its perihelion ---  $\abs{\vec v_M(0)} = 59$km/s and 
$ r_{MS}=\abs{\vec r_{MS}(0)} = 46 \cdot 10^6$km~\cite{MercuryFactSheet}, where both $\vec r$ and $\dot{\vec r}$ are perpendicular to each other. 
We thus find that $(a^2/ r_{MS}^2) = 4\cdot10^{-8}$. Furthermore $(r_S/r_{MS})=6\cdot 10^{-8}$. 
Therefore one may expect from both correction terms an effect similar in size. Furthermore, any term higher order in either $a$ or $r_S$
should be suppressed by additional six orders of magnitude and thus can be safely neglected.
The parameters $\alpha$ and $\beta$ could now either be extracted from a fit to data or be calculated from the underlying
theory --- here General relativity with the result
\begin{equation}\label{eq:art-prediction}
	\alpha = 0 \, , \quad \beta = 3 \, .
\end{equation}

However, since we have only a single number to study, the angular motion of the perihelion,
we can only fix either $\alpha$ or $\beta$.
...
To extract the linear dependence of the perihelion motion depending on $\alpha$ and $\beta$, we recommend to set one parameter to zero and vary the other.
Both parameter have similar effects on the perihelion motion.
As pointed out in Eq. \ref{eq:art-prediction}, general relativity predicts $(\alpha=0, \beta=3)$.
...
\item repeat the above points for different values of $\alpha$ and $\beta$ to confirm the linear dependence of $\delta \Theta (\alpha, \beta)$;
\item interpolate $\delta \Theta (\alpha, \beta)$ to $(\alpha=1,\beta=0)$ or $(\alpha=0,\beta=3)$ using that $\delta \Theta (0,0) = 0$ (modulo numerical errors).
\end{itemize}
...
To extract the perihelion motion at $\alpha = 0$ and $\beta = 3$, we use the fact that $\delta \Theta$ is zero for $\alpha =0 = \beta$ --- if one could completely eliminate numerical errors.
...
\begin{equation}
	\delta\Theta (\alpha=0, \beta=3) = \frac{10.6^{\degree}}{10^6} \times 3 = 0.116''
	\, .
\end{equation}
...
or
\begin{equation}
\delta \Theta \simeq 2\pi\left(\frac{a^2}{\bar r^2}\right) = (\pi \times 5 \cdot 10^{-8}) \ \mbox{rad} = (\cdot 10^{-5})^o = (4\cdot 10^{-2}) ^{''} \ ,
\end{equation}
