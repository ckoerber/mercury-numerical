...
\todo{Or would it be better to write: $\ddot{x} = \frac{c^2}{2}\left(\frac{r_S}{r^2}\right)$ and $\ddot{\vec x} = - \ddot{x} \frac{\vec{x}}{r}$?}
\begin{equation}
...
predictions of General Relativity to this particular observable. \todo{citation here}
...
\todo{Maybe we should rename $a_M$ to avoid confusion about the fact, that it is not an acceleration?}
\begin{equation}
...
Finally, we can express the evolution by a \texttt{while}-loop \todo{Should we also add  e.g. T=2*TM here?}
\begin{lstlisting}
t     = 0
alpha = 0
...
\end{lstlisting}
The class sphere will represent mercury and the sun in the simulation \todo{The pictures on the next page are done with sun radius 1.5 and mercury radius 0.5. Would it be ok, if we change the following radii to these values?}
\begin{lstlisting}
...
\end{lstlisting}
Next we tell python that the objects shall be drawn from this point on \todo{For some reason display does not work for me. However I like black as background color anyway. Would it be ok, if we leave out the display and change the color of the trajectory to white?}
\begin{lstlisting}
# display the objects from now on
display(background=color.white)
\end{lstlisting}
and last but not least, instead of updating the object-unrelated vectors in the while loop, we update the object-related vectors which will eventually be drawn \todo{changed rate ok?}
\begin{lstlisting}
t     = 0
alpha = 0
# update the objects
...
If the starting values are chosen as advised in the previous section, then the students should end up with a trajectory as depicted in figure \ref{fig:MercuryOrbit}. To get the perihelion motion, the term from equation (\ref{eq:newton_art}) has to be be added by choosing $\alpha=0$. This is a good opportunity to let the students play with the size of $\alpha$ and get a feeling for its impact on the trajectories. However, it might be advisable to not start out with the correct mercury values but a more excentric trajectory by choosing e.g. $r_{MS}(0)=6R_0$. Then the perihelion motion is better visible. This is illustrated in figure \ref{fig:MercuryOrbit}, where we used a value of $\alpha=10^6$.
...
\end{itemize}
\todo{I changed the text here in an effort to make the purpose and structure of this section more clear. However we can change it back, if you prefer the old version.}\\
...
To find out when the minimum is reached, the last two positions \texttt{vec\_r\_last} and \texttt{vec\_r\_before\_last} have to be known. If the length of the last vector is smaller than that of the previous one as well as the current vector, one has passed the perihelion and its position is given by \texttt{vec\_r\_last}. Suppose we want to save the position of the perihelion for a certain number of turns (here e.g. \texttt{max\_turns}=10) in a list \texttt{list\_perih}. \todo{I am not sure this is the best way to do this, since I need to introduce yet another concept (lists). However I could not think of an elegant solution...}
The implementation can be realised as follows \todo{I have a problem here, since I cannot use M.pos in this way. For me M.pos seems to be a pointer, because after one turn \texttt{vec\_r\_before\_last}, \texttt{vec\_r\_last} and M.pos all have the same value always. Therefore I need to use another variable and set M.pos to this value in the end. Could you check, whether it is the same for you? If yes, I will change the code accordingly, if not we have to compare codes again...}
\begin{lstlisting}
# set up vectors
vec_r_last = vec_rM0
turns      = 0
...
\begin{figure}[htb]
	...
	\includegraphics[width=.7\textwidth]{figs/precession_growth.pdf}
	\caption{\label{fcc3}Precession growth of perihelion for $\delta\theta = \delta\phi$ in degrees over number of turns for different time steps (color).  One should pay special attention to the oscillations within the growth for relatively large time steps.}
\end{figure} \todo{Could you redo this figure with some bigger time steps? Could we call the precession growth $\Theta(\alpha)$ instead of $\delta\Theta$? I think this would fit better with our notation so far...}
...
\todo{It would be nice if they could also get a quantitive estimate of the error, what do you think would be best? Maybe they can just look at the perihelion motion for $\alpha=0$ and take that as an estimate of the systematic error?}
