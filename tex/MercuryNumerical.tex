...
Now we are in the position to calculate location and velocity of the planet at $t_0+\Delta t$ using the following commands \footnote{%
	...
	Thus, the order of updating $a_{MS}$, $v_M$ and $r_M$ is not unique, but all correct prescriptions lead to the same result for sufficiently small $\Delta t$.
...
\begin{figure}[htb]
	...
	\caption{\label{fig:MercuryOrbit}%
		...
		All computations where executed with $\Delta t = 2 v_{M}(0) / c_a / 20$.
...
Consider figure \ref{fcc3}, where the angle of the perihelion relative to the $y$-axis is shown for the first $10$ turns for different choices of $\Delta t$ and $\alpha$.
...
In figure \ref{fig: LargeDT} the mercury trajectory for $\Delta t=2 * v_M(0) / c_a \cdot 5$ is shown.
...
\begin{figure}[htb]
	...
	\caption{\label{fig: LargeDT}%
		Deviation of the trajectory from precise numeric result (black line; $\Delta t=2 v_M(0) / a_M / 20$) for large time steps (red line; $\Delta t=2 v_M(0) / a_M \times 2$).
