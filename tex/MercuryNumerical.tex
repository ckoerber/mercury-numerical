...
Their detailed study needs a deep understanding of Differential Geometry. For the course presented here, however, very little math is necessary, such that high school students familiar to derivatives will benefit from it.
...
Finally, the principle of dimensional analysis is presented as a tool not only to estimate some effect as well as the 
expected accuracy of a calculation, but also to cross check whether the results of the simulation are sensible.
...
However, we put also this discussion into the paper not only for completeness but also to provide an additional 
challenge to the more advanced students.
...
Clearly, a relation as Eq.~({\ref{eq:check}}) can only provide guidance and can not replace a careful numerical check of the solutions: A valid result has to be insensitive the concrete value chosen for $\Delta t$ --- in particular replacing $\Delta t$ by $\Delta t/2$ should not change the result significantly. This issue will
...
they should discover that values of $\alpha$ of the order of $10^6$ are necessary to get a nicely visible effect.
...
This finalizes that basic course ---
for the not so ambitious students this might be a good point to stop. Already up to here the students should have
learned a lot about numerical physics, but the load of details was hopefully not too high. Moreover they have already the nice
achievement that they were able to simulate numerically a perihelion motion. The chapters to come are increasingly technical
...
As explained above [and further discussed in chapter~\ref{sec:analysis}] the natural value for $\alpha$ (and $\beta$)  would be of the order of $1$.
...
On the other hand as soon as the value of $\alpha$ or $\beta$ get too large the effective ansatz to use only the leading terms 
in the $(r_s/r)$ and $(r_a/r)^2$ expansion is no longer justified and the method of extraction might get unreliable.
We therefore recommend for the simulations used to extract the final result for choose the parameters at most of the order of $10^5$ --- this
value will better justified in chapter~\ref{sec:analysis}].

The students should convince themselves of the linear relation between the angles $\delta \Theta$ and the paramters $\alpha$ or $\beta$.
...
Note that, when comparing the results of the simulation with the experimental result, one Earth year corresponds to $T_E/T_M \approx 365~d/88~d\approx 4.15$ Mercury years.
Therefore, the perihelion motion per 100 earth years is $0.104''\cdot 415\approx 43.2''$.
This is fully consistent with the experimental value of $\delta\Theta_{\mathrm{exp}} = (42.56\pm 0.94)''$ (see Eq.~(\ref{delT})).
...
However, contrary to the correct result $\delta\Theta$ deviates from zero for $\alpha = \beta = 0$ for all $\Delta t$.
...
The absolute value of the numerical error mostly depends on the time step $\Delta t$ and is roughly independent on the values of $\alpha$ and $\beta$.
Therefore, it is more desirable to pick large values for $\alpha$ and $\beta$, because this increases the absolute value of the perihelion
...
As can be nicely seen in the figure: For smaller values of $\beta$, the relative numerical uncertainty grows, while for larger 
...
of order one.
One expects that there must be a physics reason for unnatural parameters to appear.
Employing for the problem at hand the average distance Mercury-Sun $\bar r=6\times 10^7\,\mathrm{km}$, the concept on 
...
Newtonian dynamics is simply the leading term of some more general underlying theory. 
 In particular, no new scales enter in the correction terms in addition to $r_S$ and $r_a^2$. This is by itself already an
 interesting observation.
 In case of General Relativity $\beta=3$ is a natural value 
while $\alpha$ vanishes. This pattern is a therefore a non-trivial prediction of General Relativity and one should expect
...
from the expectations of the dimensional analysis, there must be either a dynamical reason in the physical system that
is worth to identify --- or the code underlying the simulation has an error. 

It is even possible to push the idea of naturalness further to estimate the intrinsic uncertainty of a given study. 
For the problem at hand we identified the expansion parameters $(r_s/r)$ and $(r_a/r)^2$ both being of order $10^{-7}$.
Therefore, as long as natural parameters are employed in the simulation one expects corrections to be suppressed by
seven orders of magnitude since those need to scale as $(r_s/r)^2$, $(r_a/r)^4$, or $(r_s r_a^2/r^3)$.
...
for such large values of $\beta$ the effective expansion parameter is $\beta(r_a/r)^2\sim 10$\%. This is the 
...
In this work a course is presented that should enable students to understand quantitatively the perihelion motion of Mercury
...
the impacts of different potentials on the trajectories and some basic features of numerical studies.
